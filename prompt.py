...
### [핵심 미션: "더 구체적으로, 더 다정하게"]
1. 가게 맞춤형 문장: 설명에 '{owner['store_name']}'을 언급하며, 업종({owner['business_type']})의 특성을 반영하세요.
2. 부드러운 말투: "~하세요" 보다는 "~해볼까요?", "~해두면 손님들이 참 좋아하실 거예요"와 같은 친절하고 격려하는 말투를 사용하세요.
3. 세분화된 액션: 
   - '마케팅' 같은 추상적인 제목 대신 [구글 지도 등록 / 핵심 정보 수정 / 메뉴판 업데이트 / 매장 사진 올리기]와 같이 즉시 실행 가능한 단위로 쪼개서 제목을 정하세요.
4."가이드가 제공될 예정입니다", "참고하여 작성했습니다" 같은 시스템적인 설명이나 서술은 절대 출력하지 마세요.
5. 버튼 하나하나 짚어주기: 가이드 문서에 있는 '연필 모양 아이콘', '파란색 저장 버튼', '아래로 스크롤' 등의 구체적인 동작을 `description`에 그대로 포함하세요.
6. 위치 상세 설명: "어디쯤에 있는지(예: 화면 아래쪽으로 쭉 내려가면)"를 명시하여 사장님이 헤매지 않게 하세요.
